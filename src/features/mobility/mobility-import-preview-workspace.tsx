...
type MappingDialog = { kind: "partner" | "unit" | "country"; row: StudentMobilityImportRow } | null;
...
type MappingEditorDialogProps = {
  ...
  onClose: () => void;
  onCreated: (kind: "partner" | "unit" | "country", option: { id: string; nameTh: string | null; nameEn: string | null; code?: string | null; iso2?: string | null; iso3?: string | null }) => void;
};
...
function MappingEditor({ dialog, countries, onClose, onCreated }: MappingEditorDialogProps) {
  ...
  const isCountry = activeDialog.kind === "country";
  const defaultName = isPartner
    ? activeDialog.row.institutionName
    : isCountry
      ? activeDialog.row.sourceCountry?.name || ""
      : activeDialog.row.sourceUnit?.name || "";
  ...
  async function create(formData: FormData) {
    ...
    const payload = isPartner
      ...
      : isCountry
        ? { kind: "country", iso2: String(formData.get("iso2") || ""), iso3: String(formData.get("iso3") || ""), nameTh: String(formData.get("nameTh") || ""), nameEn: String(formData.get("nameEn") || "") }
    ...
    const data = await response.json() as { id?: string; nameTh?: string | null; nameEn?: string | null; code?: string | null; iso2?: string | null; iso3?: string | null; error?: string };
    setSaving(false);
    if (!response.ok || !data.id) { setError(data.error || "สร้างข้อมูลอ้างอิงไม่สำเร็จ"); return; }
    onCreated(activeDialog.kind, { id: data.id, nameTh: data.nameTh || null, nameEn: data.nameEn || null, code: data.code, iso2: data.iso2, iso3: data.iso3 });
  }

  return <section className="mobility-import-create"><div className="mb-3"><h3>{isPartner ? "เพิ่มองค์กรคู่ความร่วมมือ" : isCountry ? "เพิ่มประเทศ" : "เพิ่มหน่วยงาน ม.พะเยา"}</h3><p>{isPartner ? "องค์กรใหม่จะอยู่สถานะรอตรวจสอบ และถูกเลือกให้รายการนี้ทันที" : isCountry ? "เพิ่มเฉพาะเมื่อยืนยันว่าไม่มีในข้อมูลอ้างอิง และใช้รหัส ISO มาตรฐาน" : "ใช้เมื่อยืนยันแล้วว่าไม่มีหน่วยงานนี้ในรายการอ้างอิง"}</p></div><form action={create} className="mobility-import-create-form"><label>ชื่อภาษาไทย<Input name="nameTh" defaultValue={isPartner ? "" : defaultName} required={!isPartner} /></label><label>ชื่อภาษาอังกฤษ<Input name="nameEn" defaultValue={isPartner ? defaultName : ""} required={isCountry} /></label>{isPartner ? <label className="is-wide">ประเทศ<select name="countryId" defaultValue=""><option value="">ยังไม่ระบุ</option>{countries.map((country) => <option key={country.id} value={country.id}>{country.name_th} ({country.name_en})</option>)}</select></label> : isCountry ? <><label>ISO-2<Input name="iso2" required /></label><label>ISO-3<Input name="iso3" required /></label></> : <label className="is-wide">รหัสหน่วยงาน (ถ้ามี)<Input name="code" defaultValue={defaultCode} /></label>}{error ? <p className="is-wide text-sm text-destructive">{error}</p> : null}<div className="mobility-import-create-actions is-wide"><Button type="button" variant="outline" onClick={onClose}>ยกเลิก</Button><Button type="submit" disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : <Plus />}{saving ? "กำลังบันทึก" : "สร้างและเลือก"}</Button></div></form></section>;
}
...
export function MobilityImportPreviewWorkspace({ access }: { access: CurrentUserAccess }) {
  ...
  const [extraCountries, setExtraCountries] = useState<Array<{ id: string; iso2: string; name_th: string; name_en: string }>>([]);

  async function buildPreview() {
    if (!file) { setError("เลือกไฟล์ .xlsx ก่อนสร้าง preview"); return; }
    setError(undefined); setLoading(true); setPreview(undefined); setMappings({}); setExtraPartners([]); setExtraUnits([]); setExtraCountries([]); setFilter("attention");
  ...
  async function createStaging() {
    ...
    if (remainingErrors > 0) {
      setError(`ยังมี ${remainingErrors} รายการที่ต้องแก้ mapping ก่อนสร้าง staging`);
      setFilter("error");
      return;
    }
    setStaging(true);
    ...
    const response = await fetch("/api/mobility/import-stage", { method: "POST", body });
    const payload = await response.json() as { batchId?: string; error?: string };
    setStaging(false);
    if (!response.ok || !payload.batchId) {
      setError(payload.error || "สร้าง staging ไม่สำเร็จ");
      return;
    }
    router.push(`/mobility/import/${payload.batchId}`);
  }
  ...
  const allCountries = preview ? [...preview.referenceOptions.countries, ...extraCountries] : [];

  function setMapping(projectId: string, change: Mapping) { setMappings((current) => ({ ...current, [projectId]: { ...current[projectId], ...change } })); }

  function created(kind: "partner" | "unit" | "country", option: { id: string; nameTh: string | null; nameEn: string | null; code?: string | null; iso2?: string | null; iso3?: string | null }) {
    if (!dialog) return;
    ...
    else if (kind === "country") { setExtraCountries((current) => [...current, { id: option.id, iso2: option.iso2 || "", name_th: option.nameTh || "", name_en: option.nameEn || "" }]); setMapping(dialog.row.legacyProjectId, { countryId: option.id }); }
  ...
  return <main className="module-main"><div className="module-page-heading"><div><Link className="back-link" href="/mobility"><ArrowLeft />กลับไปรายการ Mobility</Link><h1>ตรวจและแก้ Mapping ข้อมูล Mobility นิสิต</h1><p>เลือกข้อมูลอ้างอิงที่ถูกต้องก่อนสร้าง staging batch — การเลือกในหน้านี้ยังไม่เขียนข้อมูลโครงการหรือผู้เข้าร่วมลง Supabase</p></div></div>
    <section className="module-list-card p-6 space-y-5"><div className="flex items-start gap-3"><span className="mou-row-icon"><FileSpreadsheet /></span><div><h2 className="text-lg font-semibold">เลือกไฟล์ฐานข้อมูลเดิม</h2><p className="text-sm text-muted-foreground">รองรับ Excel ที่มีชีต MOBILITY_PROJECT, MOBILITY_PARTICIPANT, COUNTRY_MASTER และ UP_UNIT_MASTER</p></div></div><div className="mobility-import-file-actions"><label className="mobility-import-file-picker"><Upload /> เลือกไฟล์ Excel<input aria-label="ไฟล์ Excel สำหรับตรวจข้อมูล" type="file" accept=".xlsx" onChange={(event) => setFile(event.target.files?.[0])} /></label><span className={file ? "mobility-import-file-name is-selected" : "mobility-import-file-name"}>{file ? file.name : "ยังไม่ได้เลือกไฟล์"}</span><Button type="button" disabled={loading} onClick={buildPreview}>{loading ? <Loader2 className="animate-spin" /> : <Upload />}{loading ? "กำลังตรวจข้อมูล..." : "สร้าง preview"}</Button></div>{file ? <p className="text-sm text-muted-foreground">ขนาดไฟล์ {Math.ceil(file.size / 1024 / 1024)} MB</p> : null}{error ? <p className="text-sm text-destructive">{error}</p> : null}</section>
