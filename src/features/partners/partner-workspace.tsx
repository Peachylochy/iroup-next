...
import { Building2, CheckCircle2, CircleAlert, Clock3, Plus, Search } from "lucide-react";
...
export function PartnerWorkspace({ access, partners, viewer }: Props) {
  ...
  return <WorkspaceChrome access={access} viewer={viewer} title="องค์กรคู่ความร่วมมือ" activePath="/mou/organizations" query={query} onQueryChange={setQuery}>
    <main className="module-main">
      ...
      <section className="module-list-card" aria-labelledby="partner-list-title"><div className="module-list-toolbar"><div><h2 id="partner-list-title">รายชื่อองค์กร</h2><p>ค้นหาก่อนสร้างรายการใหม่เพื่อป้องกันข้อมูลซ้ำ</p></div><label className="module-search"><Search aria-hidden="true" /><span className="sr-only">ค้นหาองค์กร</span><Input type="search" placeholder="ค้นหาชื่อองค์กรหรือประเทศ" value={query} onChange={(event) => setQuery(event.target.value)} /></label></div>
